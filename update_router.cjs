...
// 2. Extract out old components
const startIdx = content.indexOf('function PlayerGate() {');
const endIdx = content.indexOf('function LoginRoute() {');

const newComponents = `function ItemDetailsGate() {
...
function ItemDetailsRoute() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const itemDetails = await fetchItemDetails(serverUrl, session.userId, itemId, session.accessToken);
        if (cancelled) return;
        setDetails(itemDetails);

        const similar = await fetchSimilarItems(serverUrl, session.userId, itemId, session.accessToken, 8).catch(() => []);
        if (cancelled) return;
        setSimilarItems(similar);

        if (itemDetails.type === 'Series') {
          const seasonsList = await fetchSeasons(serverUrl, session.userId, itemId, session.accessToken).catch(() => []);
          if (cancelled) return;
          ...
          if (seasonsList.length > 0) {
            ...
            const episodesList = await fetchEpisodes(serverUrl, session.userId, itemId, firstSeason, session.accessToken).catch(() => []);
            if (cancelled) return;
  ...
  useEffect(() => {
    if (!session || !itemId || !selectedSeasonId || details?.type !== 'Series') return;
    let cancelled = false;
    fetchEpisodes(serverUrl, session.userId, itemId, selectedSeasonId, session.accessToken)
